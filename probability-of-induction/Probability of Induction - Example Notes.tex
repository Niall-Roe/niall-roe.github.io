...
FOr now lets make it a coming soon. 
...
##15
highlight the bit that asks the user to click on the chart. 

##17

this is great, but lets do a few things. 

First, fix the slider, it is okay if it just goes from 50 to 99. 

Second, for the binomal chart, would it be possible to heatmap the vertical lines according to how many times they have been drawn?

Third, lets have a button that sets it to peirce's example (true proprting is a third and sample size is 4. can add a little text that says something like, while we can never draw a sample that reflects the true proportion, we can be confident that our error bounds will never the less contain it [percent] of the time.

Fourth, have rescale the charts on by default. 

fifth... would it make sense to show the empirical distribution too? not sure, but curious to hear your thoughts. 

##16 - hypothesis test. 

we can cut the analysis results bit. as i think that should all be covered in the text already? maybe could there be another line along the difference line that is perferated in probable error sum sized perferations, showing that the difference is nx the sum of the probable errors? This could be indicated with colour, as we already have colours for the probable error bounds. (so each unit line could be x% blue and y% red, and equal in length to the sum of the PEs.) 

that might mess with stuff a bit when we shift the Con. Lvl. but how should we handle that? it could become the sum of the blank% CL rather than PE, and have the text update as well. 

(Also, what is the reasoning behind using the sum of the PEs? is that like any modern technique?


##18

need to make the tolerably certain bit clearer. 

Please:

Cut the explainer paragraph at the bottom (as it just mimics the text)

have this bit of the text update live: "Thus we should be tolerably certain of not being in error by more than one ball in 100."

it should update as you change the sample size and proportion. 

Part of the point of this example is that when the true proprtion is near the extreme, the same PE value is more useful than if the dist is in the middle. 

We could have a button that sets it to the same sample size but puts the proportion to .50. it shoudl show that the X in 100 updates. 

shoudl it also show the PE around the bar you have clicked? is that the relevant PE?

when chart is scaled to the 0-1 number line, could have vertical lines under the graph showing the size of the PE at each point. this would show the point that they are different for the extremes. 



##20 - cretan induction

this one needs to be slightly tweaked to reflect pierce's reasoning. 

Have it go over this text:

"But when I reason in the opposite direction: Minos, Sarpedon, Rhadamanthus, Deucalion, and Epimenides, are all the Cretans I can think of; But these were all atrocious liars, Therefore, pretty much all Cretans must have been liars;I do not in the least know how often such reasoning would carry me right. On the other hand, 20what I do know is that some definite proportion of Cretans must have been liars, and that this proportion can be probably approximated to by an induction from five or six instances. Even in the worst case for the probability of such an inference, that in which about half the Cretans are liars, the ratio so obtained would probably not be in error by more than 1/6. "

The sample size is tied to the number of names. "Minos, Sarpedon, Rhadamanthus, Deucalion, and Epimenides," The slider that currently does sample size is actually the number of samples. 

As the sample size changes, change the names. Have a set of 100 greek names (make and female, ancient if possible). When a sample is drawn, pick n creteans, and update the names to be those names in the list. Colour coordinate them. In the background, it should be decided which ones are liars and which tell the truth, so that if the boxes show this many liars and those many truth tellers, the names reflect the same. as you change the proportion, it changes who is a liar or not. 

have a button for take five samples. 

i think this should fix the weird numbers we were getting when you hit reveal the proportion. please double check all that. 
...
this one is cool...  for now lets hide the "Weighted by" view. 

##33

the "take one more drawing" button does not work on the probable error view. either it should or it sohuld only appear in the drawings view. 
