...
Shhould be combined with 23. see below. 

##23 - 

link it to this text:

leads simply to the conclusion that reasoning from past to future experience is absolutely worthless. In fact, the moment that you assume that the chances in favor of that of which we are totally ignorant are even, the problem about the tides does not differ, in any arithmetical particular, from the case in which a penny (known to be equally likely to come up heads and tails) should turn up heads m times successively. In short, it would be to assume that Nature is a pure chaos, or chance combination of independent elements, in which reasoning from one fact to another would be impossible; and since, as we shall hereafter see, there is no judgment of pure observation without reasoning, it would be to suppose all human cognition illusory and no real knowledge possible. It would be to suppose that if we have found the order of Nature more or less regular in the past, this has been by a pure run of luck which we may expect is now at an end."

the point is that in assuming there are even odds, we are making a factual assumption. we are assuming that the world is like a fair coin, but they making the mistake of treating any run of heads or tails as evidence of bias / regularity. 

really, the conceptualist should be treating this as asking which urn the coin came from, and hoping that it lines up over the urn for "fair coin"

but the next bit shows that this "is something which we should have a right to talk about if universes were as plenty as blackberries, if we could put a quantity of them in a bag, shake them well up, draw out a sample, and examine them to see what proportion of them had one arrangement and what proportion another. "


## new example

for this text " The relative probability of this or that arrangement of Nature is something which we should have a right to talk about if universes were as plenty as blackberries, if we could put a quantity of them in a bag, shake them well up, draw out a sample, and examine them to see what proportion of them had one arrangement and what proportion another." 


##24

we should set this one up maybe as a bayesian vs frequentist thing?
...
Claude: done — every edit below is now in the page. Where an example was marked
to be held back the code is untouched; only what its trigger opens has changed,
so nothing has to be unpicked to put it back.
...
Peirce's chance is what we now call the odds. A chance of 81/19 is odds of 81:19 on; an even chance, 1:1, is evens.
...
antecedents are defined and sometimes both hold. This makes the question possible to answer, but doesn't give that answer.  The two arguments leave the proportion anywhere
...
observe A2 twice as often and it moves even though  neither argument has changed.
...
On these drawings we calculate two numbers. The first is the
proportion white among the drawings. The second is the probable error: the
amount by which the first is as likely as not to be wrong. Initially, drawing more beans
...
0  ½  1
You have read no letters, so the rule of succession says: ½ — (0 + 1) / (0 + 2).
It would answer the same of any question whatever. The paper is in fact 0.619 consonant.
...
to all black.
Imagine there are A hundred and one urns, the kth holding letters of which k in a hundred are
consonants. Pretend that his paper is written in just consonants and vowels, without worrying about the specific consonant or vowel. We will proceed like the conceptualist. Read some of the paper and try to determine the probability that it was drawn from a given urn. 
...
Nothing has been read, so by the principle of indifference (or rule of succession) every urn is as likely as every other and the distribution
is flat. This reflects an assumption and is the result of ignorance. Later findings built on this flat distribution are only as good as their foundation. 
...
{Hide this for now (dont delete the code, just hide it). user side, replace it with a  coming soon note} 
