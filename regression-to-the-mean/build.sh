...
#
# Order matters. 01 opens <html><head><style> and <body>; 99 closes </body></html>.
#
...
PARTS=(
  ...
  src/02_article.html # title, the argument, control/figure mounts, footer
  src/03_lib.js       # dnorm/pnorm/qnorm, canvas renderer, DOM helpers — shared with severity
  src/04_core.js      # the apparatus as a model: the two banks, the chutes, the population
  src/05_board.js     # the apparatus as a picture, and the shot moving through it
  src/06_panel.js     # spread by generation, and the readout under both figures
  src/07_ui.js        # controls, the three buttons, the mount
  src/99_close.js     # initial build(), closes </body></html>
)
